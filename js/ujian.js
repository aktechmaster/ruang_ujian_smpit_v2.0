...
⚠️ Gagal Memuat Soal Ujian
Detail Error: ${error.message}

Silakan pastikan berkas JSON soal sudah ada di repositori untuk tingkat kelas ${tingkatKelas} dan mapel ${mapelUjian}.
...
teksPertanyaanBersih = teksPertanyaan;
    ...
    htmlSoal += `
${index + 1}.
${teksPertanyaanBersih}

`;

    // Render jika terdapat objek gambar khusus di file JSON
    if (soal.gambar || soal.image) {
        let srcGambar = soal.gambar || soal.image;
        htmlSoal += `
`;
}
...
// Otomatis memperbaiki typo 'images_MIK_9' menjadi 'images_MTK_9'
htmlSoal = htmlSoal.replace(/images_MIK_9/gi, 'images_MTK_9');

// Otomatis menyelipkan prefix folder 'Images/STS_1/' ke seluruh tag image
htmlSoal = htmlSoal.replace(/src=["']\/?(?:Images\/STS_1\/)?images_/gi, 'src="Images/STS_1/images_');
